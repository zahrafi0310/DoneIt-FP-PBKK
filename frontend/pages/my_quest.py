...
def show():
    ...
    try:
        ...
        for s in submissions:
            qid = s["quest_id"]
            if qid not in sub_map or s["submitted_at"] > sub_map[qid]["submitted_at"]:
                sub_map[qid] = s
    except:
        sub_map = {}
    ...
    for aq in my_quests:
        ...
        if sub_status == "pending":
            ...
            <div style="background:#fff8e1; border-radius:10px; padding:0.6rem 1rem;
                        color:#b8860b; font-size:0.88rem; font-weight:600; margin-bottom:0.6rem;">
            ...
            <div style="background:#d4f7d4; border-radius:10px; padding:0.6rem 1rem;
                        color:#276b27; font-size:0.88rem; font-weight:600; margin-bottom:0.6rem;">
            ...
            <div style="background:#ffd5d5; border-radius:10px; padding:0.6rem 1rem;
                        color:#8b0000; font-size:0.88rem; font-weight:600; margin-bottom:0.4rem;">
                Submission ditolak.{f" Alasan: {reason}" if reason else ""}
            </div>
            """, unsafe_allow_html=True)
            col1, col2 = st.columns([1, 1])
            with col1:
                if st.button("Submit Ulang", key=f"resubmit_{aq['id']}", type="primary", use_container_width=True):
                    st.session_state.submit_quest_id   = quest_id
                    st.session_state.submit_quest_title = q["title"]
                    st.rerun()
            with col2:
                if st.button("Batalkan Quest", key=f"cancel_rej_{aq['id']}", use_container_width=True):
